...
This script reads all Geonections_*.json files, extracts panoId data,
...
async def download_pano_image(browser_pool, tile, api_key, output_dir, http_port, skip_direct_download=False):
    ...
    try:
        ...
        base_url = f"http://localhost:{http_port}/scripts/download.html"
        params = {
            "key": api_key,
...
async def process_json_file_async(json_file, browser_pool, api_key, http_port, skip_direct_download=False):
    ...
    try:
        ...
        output_dir = Path(__file__).parent.parent / "public" / "pano" / "img"
...
def prune_orphaned_images(delete_orphans=False):
    ...
    img_dir = script_dir.parent / "public" / "pano" / "img"  # Images go in public/pano/img/
    parent_dir = script_dir.parent
    
    # Find all Geonections JSON files
    grids_dir = parent_dir / "public" / "puzzles"
    json_files = list(grids_dir.glob("*.json"))
    
    if not json_files:
        print("No *.json files found in puzzles directory")
        return
...
async def main_async():
    ...
    import requests
    try:
        response = requests.get(f"http://localhost:{port}/scripts/download.html", timeout=5)
        if response.status_code == 200:
